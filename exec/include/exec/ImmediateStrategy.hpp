...
public:
    ...
    [[nodiscard]] bool is_paused() const noexcept override { return paused_.load(std::memory_order_relaxed); }
...
private:
    ...
    std::atomic<std::uint64_t> order_seq_{0};   ///< monotonic counter for client_order_id generation
};
